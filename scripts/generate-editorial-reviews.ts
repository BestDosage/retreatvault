...
async function main() {
  ...
  console.log(`Generating editorial reviews for top ${limit} retreats...`);
  console.log(`Mode: ${shouldInsert ? "Generate + Insert as draft" : "Generate JSON only"}\n`);
  ...
  console.log(`Fetched ${retreats.length} retreats\n`);

  // Generate reviews sequentially (rate limiting)
  const reviews: EditorialReview[] = [];
  for (let i = 0; i < retreats.length; i++) {
    const retreat = retreats[i];
    const pct = Math.round(((i + 1) / retreats.length) * 100);
    process.stdout.write(
      `[${i + 1}/${retreats.length}] ${pct}% — ${retreat.name}...`
    );

    try {
      const review = await generateReviewWithClaude(retreat, retreats);
      reviews.push(review);
      console.log(" done");
    } catch (e: any) {
  ...
  // Write JSON output
  const outputPath = "data/editorial-reviews.json";
  if (!fs.existsSync("data")) fs.mkdirSync("data", { recursive: true });
  fs.writeFileSync(outputPath, JSON.stringify(reviews, null, 2));
  console.log(`\nWrote ${reviews.length} reviews to ${outputPath}`);

  // Optionally insert into Supabase
  if (shouldInsert && reviews.length > 0) {
    console.log("\nInserting into Supabase as drafts...");
    ...
    const { error: insertError } = await supabase
      ...
      .upsert(rows, { onConflict: "retreat_id" });

    if (insertError) {
      console.error("Insert error:", insertError.message);
    } else {
      console.log(`Inserted ${rows.length} reviews as drafts.`);
      console.log('Set status to "published" in Supabase when ready to go live.');
    }
  }

  // Summary
  console.log("\n--- Summary ---");
  console.log(`Total generated: ${reviews.length}`);
  console.log(`Failed: ${retreats.length - reviews.length}`);
  const avgLen = Math.round(
    reviews.reduce((sum, r) => sum + r.review_html.length, 0) / reviews.length
  );
  console.log(`Avg review length: ~${avgLen} chars (~${Math.round(avgLen / 5)} words)`);
  console.log(
    `\nNext: Review ${outputPath}, edit as needed, then run with --insert`
  );
}
